...
class S4TSWorker(QRunnable):
    ...
    def run(self):
        try:
            ...
            print(f"Error in S4TSWorker: {str(e)}")
        finally:
...
class ElevensLabS4TS(QMainWindow):
    ...
    def slow_flatten_wave(self) -> None:
        ...
        if self.is_recording or self.last_wave is None:
            return
        wave = self.last_wave[:]
    ...
    def stop_recording(self):
        ...
        self.recFile.close()  # Make sure to close the file
        self.is_recording = False
    ...
    def s4ts(self, file: str):
        if self.tts is None:
            self.status_bar.showMessage('API key not set. Please enter your API key.')
            return
        ...
        worker.signals.finished.connect(self.on_s4ts_finished)
        self.threadpool.start(worker)

    def on_s4ts_finished(self):
